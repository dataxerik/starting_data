...
def plot_state_borders(plt, color='black'):
    state_data = ET.parse("states.xml").getroot()
    for state in state_data:
        state_coord = [[point.attrib['lng'], point.attrib['lat']]
                       for point in state]
        x_list = [pointx for [pointx, _] in state_coord]
        y_list = [pointy for [_, pointy] in state_coord]
...
def plot_languages():
    ...
    for k in [1, 3, 5, 7]:
        ...
        for location, actual_language in cities:
            ...
            predicted_lanaguage = knn_classify(k, other_cities, location)

            if predicted_lanaguage == actual_language:
                num_correct += 1
        print("{} neighbor[s]: {} correct out of {}".format(k, num_correct, len(cities)))
...
def plot_grid(k=1):
    ...
    for longitude in range(-130, -60):
        for latitude in range(20, 55):
            predicted_langauge = knn_classify(k, cities, [longitude, latitude])

            plots[predicted_langauge][0].append(longitude)
            plots[predicted_langauge][1].append(latitude)
    ...
    plt.legend(loc=0)
    plt.axis([-130, -60, 20, 55])
    plt.title("{}-Nearest Neighbor Programming Languages")
    plt.show()
